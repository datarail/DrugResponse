...
def compute_log_dna(dna, x_dna):
    ...
    dna_upper_bounded =  [d if d <  dna_upper_bound else dna_upper_bound
                          for d in dna]
    dna_bounded = [d if d  > dna_lower_bound else dna_lower_bound for d in dna]
...
def get_g1_location(log_dna, ldrtxt, ldr_gates):
    ...
    dna_density = [np.mean(np.array(log_dna>(x - 0.2 * np.log10(2))) & np.array(
        log_dna<(x + 1.2 * np.log10(2)))) for x in xdna_loc] + dna_peaks_amp
    # Find G1 peak
    if len(xdna_loc) == 2:
...
def get_g2_location(log_dna, ldrtxt, ldr_gates, g1_loc):
    ...
    if len(xdna_loc) > 1:
        g2_loc = xdna_loc[np.argmin(np.abs((xdna_loc - (g1_loc + np.log10(2)))))]
    elif len(xdna_loc) == 1:
        g2_loc = xdna_loc[0]
    else:
...
def plot_dna_gating(dna, x_dna, ldrtxt, ldr_gates):
    ...
    g1_loc = g1_g2_pos[0]
    g2_loc = g1_g2_pos[1]
    dna_gates = [a+b for a,b in zip([g1_g2_pos[i] for i in [0, 0, 1, 1]],
...
def live_dead(ldrtxt, x_ldr, ldr_gates, log_dna=None, x_dna=None, dna_gates=None):
    ...
    if log_dna is not None :
        ...
        plt.pie([selected, others, dead], labels=['selected', 'others', 'dead'],
